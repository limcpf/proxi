...
- `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop` 훅을 사용해 시작 체크리스트, `main` 에서의 task 브랜치/worktree 수동 준비 안내, Bash 직전 main 차단, 문서 변경 상태 기록, 종료 전 `corepack pnpm run verify` 강제를 구현한다.
- 훅 상태는 비추적 경로인 `.codex/tmp/` 아래에 저장하고, 현재 작업트리의 문서 변경 fingerprint 와 마지막 검증 fingerprint 를 비교해 종료 시점 판단에 사용한다.
- worktree 권장 루트는 `/tmp/<project>-wt/` 로 두되, hook 이 이름을 정해 branch 와 worktree 를 자동 생성하지 않는다.

## 대안
- 문서와 PR 검증만으로 운영하는 방식은 세션 시작 행동을 고정하지 못해 채택하지 않았다.
- `PreToolUse` 에만 의존하는 방식은 새 작업 준비 안내와 종료 검증을 대신할 수 없어 채택하지 않았다. 대신 Bash 직전의 main 차단만 담당하게 한다.
- 모든 작업을 현재 브랜치에서 이어가는 방식은 task 간 물리적 분리가 없어 채택하지 않았다.

## 영향
- 새 세션은 시작 즉시 라우터와 기계용 목차를 다시 확인하는 컨텍스트를 받는다.
- `main` 에서 첫 작업 프롬프트가 들어오면 branch 와 worktree 를 직접 만들라는 안내를 출력하고, 사용자가 정한 경로에서 새 세션을 다시 시작하도록 강제한다.
...
- 2026-05-04 후속 수정으로 `UserPromptSubmit` 의 branch/worktree 자동 생성을 제거했다. 이제 `main` 에서는 사용자가 원하는 이름으로 직접 `git worktree add -b` 를 실행하라는 안내만 출력하고 세션을 중단한다.
