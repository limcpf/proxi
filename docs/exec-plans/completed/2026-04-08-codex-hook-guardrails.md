...
- 세션 시작, `main` 브랜치 worktree 부트스트랩, 문서 변경 상태 기록, 종료 전 검증 강제를 담당하는 Node hook 스크립트를 추가했다.
...
- 2026-05-04: `PostToolUse` 의 명령 추출과 응답 성공 판정을 다시 보강했다. 중첩 JSON, 대체 응답 필드명, `Process exited with code 0` 문자열을 처리하고, 실행 중 응답은 성공으로 기록하지 않도록 했다.
- 2026-05-04: `PreToolUse` 의 `main` Bash 차단 훅을 제거했다. `main` 에서의 작업 분리는 `UserPromptSubmit` 에서 task 브랜치와 worktree 를 만드는 시점으로 일원화하고, 종료 시점은 `Stop` 훅이 보완한다.
- 2026-05-04: `PostToolUse` 의 문서 변경 리마인드 출력을 제거했다. Bash 실행마다 반복되는 경고를 막고, `PostToolUse` 는 상태 기록과 verify fingerprint 갱신만 담당한다.
